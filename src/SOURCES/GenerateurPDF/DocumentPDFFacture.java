...
import SOURCES.UI.PanelContenuFacture;
 ...
 * @author Gateway
 */
public class DocumentPDFFacture extends PdfPageEventHelper {
    ...
    private PanelContenuFacture gestionnaireFacture;

    public DocumentPDFFacture(PanelContenuFacture panel, int action, boolean isRecu, SortiesFacture sortiesFacture) {
        try {
    ...
    private void init(PanelContenuFacture panel, int action, boolean isRecu, SortiesFacture sortiesFacture) {
    ...
    private void setBasDePage() throws Exception {
        if (this.gestionnaireFacture != null) {
            Entreprise entreprise = this.gestionnaireFacture.getParametres().getEntreprise();
            if (entreprise != null) {
    ...
    private void setLogoEtDetailsEntreprise() {
        try {
            ...
            String logo = "";
            if (this.gestionnaireFacture != null) {
                logo = (new File(this.gestionnaireFacture.getParametres().getEntreprise().getLogo())).getName();
        ...
        try {
            ...
            if (this.gestionnaireFacture != null) {
                ...
                for (Paiement paiement : listePaiement) {
                    //cumuls
                    Frais Iarticle = UtilFacture.getFrais(gestionnaireFacture.getDonneesFacture(), paiement.getIdFrais());
                    Monnaie Imonnaie = UtilFacture.getMonnaie(gestionnaireFacture.getParametres(), Iarticle.getIdMonnaie());
                    if (Iarticle != null && Imonnaie != null) {
                        String monnaie = Imonnaie.getCode();
                        totPaye += UtilFacture.getMontantOutPut(gestionnaireFacture.getParametres(), Iarticle.getIdMonnaie(), paiement.getMontant());
                        totReste += UtilFacture.getMontantOutPut(gestionnaireFacture.getParametres(), Iarticle.getIdMonnaie(), modelPaiement.getReste(paiement.getIdFrais(), -1));
                        String periode = UtilFacture.getPeriode(gestionnaireFacture.getParametres(), paiement.getIdPeriode()).getNom();
                        double restePeriode = UtilFacture.round((modelPaiement.getReste(paiement.getIdFrais(), paiement.getIdPeriode())), 2);
                        setLigneTabReleve(tableReleve, monnaie, UtilFacture.getDateFrancais(paiement.getDate()), paiement.getNomFrais(), paiement.getReferenceTransaction(), periode, paiement.getMode(), i, UtilFacture.round(paiement.getMontant(), 2), restePeriode);
                        i++;
                    }
                }
            ...
            document.add(getParagraphe("En lettre : " + (UtilFacture.getMontantLettres(totPaye, monnaie)), Font_TexteSimple_petit_Gras, Element.ALIGN_RIGHT));
    ...
    private void setTableauDetailsPlanPaiementEchelonne() {
        try {
            ...
            if (this.gestionnaireFacture != null) {
                ...
                Vector<Echeance> listeEcheances = modeEcheance.getListeData();
                int i = 0;
                double totDu = 0;
                double totPaye = 0;
                for (InterfaceEcheance echeance : listeEcheances) {
                    setLigneTabEcheance(tableEchances, echeance.getNom(), UtilFacture.getDateFrancais(echeance.getDateInitiale()), UtilFacture.getDateFrancais(echeance.getDateFinale()), modeEcheance.getStatus(i), i, UtilFacture.round(echeance.getMontantDu(), 2), UtilFacture.round(echeance.getMontantPaye(), 2));
                    totDu += echeance.getMontantDu();
                    totPaye += echeance.getMontantPaye();
                    i++;
                }
                setDerniereLigneTabEcheance(tableEchances, UtilFacture.round(totDu, 2), UtilFacture.round(totPaye, 2));
            } else {
                for (int i = 0; i < 10; i++) {
                    setLigneTabEcheance(tableEchances, "Tranche n°" + (i + 1), UtilFacture.getDateFrancais(new Date()), UtilFacture.getDateFrancais(new Date()), "Expirée depuis " + (i + 1) + " jour", i, 100, 50);
                }
    ...
    private void setTableauSynthese() {
        try {
            ...
            if (this.gestionnaireFacture != null) {
                ...
                ModeleListeFrais mfacture = this.gestionnaireFacture.getModeleListeArticles();
                if (mPaiement != null & mfacture != null) {
                    ...
                    setLignesTabSynthese(tableSynthese, 0, this.gestionnaireFacture.getParametres().getMonnaieOutPut().getCode(), 0, 0, 0, Tttc, Tpaye, Tsolde);
                }
            } else {
    ...
    private void setTableauDetailsArticles() {
        try {
            ...
            if (this.gestionnaireFacture != null) {
                ModeleListeFrais modelArticle = this.gestionnaireFacture.getModeleListeArticles();
                Vector<Frais> listeArticles = modelArticle.getListeData();
                int i = 0;
                for (Frais article : listeArticles) {
                    setLigneTabArticle(tableDetailsArticles, article, i);
                    i++;
                }
                setDerniereLigneTabArticle(tableDetailsArticles, 0, 0, 0, 0, modelArticle.getTotal_TTC());
            } else {
                for (int i = 0; i < 5; i++) {
    ...
    private void setLigneTabArticle(PdfPTable tableDetailsArticles, Frais article, int i) {
        ...
        if (Im != null) {
            ...
            tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(1) + "", 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
            tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(0) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
            tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(0) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
            tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(0) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
            tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(0) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
            tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(article.getMontantDefaut()) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
        }
    ...
    private void setLigneTabReleve(PdfPTable tableDetailsArticles, String monnaie, String date, String NomArticle, String reference, String periode, int mode, int i, double montant, double reste) {
        ...
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(montant) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(reste) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
    }
    ...
    private void setLigneTabEcheance(PdfPTable tableEchances, String nomEcheance, String dateDebut, String dateFin, String progression, int i, double montantDu, double montantPaye) {
        ...
        tableEchances.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(montantDu) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
        tableEchances.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(montantPaye) + " " + monnaie, 0.2f, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));
    }
    ...
    private void setDerniereLigneTabEcheance(PdfPTable tableEchances, double totalDu, double totalPaye) {
        ...
        tableEchances.addCell(getCelluleTableau("", 0, BaseColor.LIGHT_GRAY, null, Element.ALIGN_LEFT, Font_TexteSimple_Gras));
        tableEchances.addCell(getCelluleTableau("", 0, BaseColor.LIGHT_GRAY, null, Element.ALIGN_LEFT, Font_TexteSimple_Gras));
        tableEchances.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(totalDu) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        tableEchances.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(totalPaye) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
    }
    ...
    private void setLignesTabSynthese(PdfPTable tableau, float borderwidth, String monnaie, double totaRabais, double totalHt, double totalTva, double totalTTC, double totalPaye, double totalSolde) {
        if (totaRabais != 0) {
            ...
            tableau.addCell(getCelluleTableau("- " + UtilFacture.getMontantFrancais(totaRabais) + " " + monnaie, borderwidth, BaseColor.WHITE, BaseColor.RED, Element.ALIGN_RIGHT, Font_TexteSimple_Italique));
        }
        tableau.addCell(getCelluleTableau("Montant HT", borderwidth, BaseColor.WHITE, null, Element.ALIGN_LEFT, Font_TexteSimple));
        tableau.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(totalHt) + " " + monnaie, borderwidth, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));

        tableau.addCell(getCelluleTableau("Tva (16%)", borderwidth, BaseColor.WHITE, null, Element.ALIGN_LEFT, Font_TexteSimple));
        tableau.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(totalTva) + " " + monnaie, borderwidth, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple));

        tableau.addCell(getCelluleTableau("Montant TTC", borderwidth, BaseColor.WHITE, null, Element.ALIGN_LEFT, Font_TexteSimple_Gras));
        tableau.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(totalTTC) + " " + monnaie, borderwidth, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        ...
        if (totalPaye != 0) {
            ...
            tableau.addCell(getCelluleTableau("- " + UtilFacture.getMontantFrancais(totalPaye) + " " + monnaie, borderwidth, BaseColor.WHITE, BaseColor.RED, Element.ALIGN_RIGHT, Font_TexteSimple_Italique));
        }

        tableau.addCell(getCelluleTableau("Solde global", borderwidth, BaseColor.WHITE, null, Element.ALIGN_LEFT, Font_TexteSimple_Gras));
        tableau.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(totalSolde) + " " + monnaie, borderwidth, BaseColor.WHITE, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
    ...
    private void setDerniereLigneTabArticle(PdfPTable tableDetailsArticles, double punit1, double rabais, double punit2, double mntTva, double totalTTC) {
        ...
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(punit1) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(rabais) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(punit2) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(mntTva) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(totalTTC) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
    }
    ...
    private void setDerniereLigneTabReleve(PdfPTable tableDetailsArticles, String monnaie, double montant, double reste) {
        ...
        tableDetailsArticles.addCell(getCelluleTableau("", 0, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        tableDetailsArticles.addCell(getCelluleTableau("", 0, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
        tableDetailsArticles.addCell(getCelluleTableau(UtilFacture.getMontantFrancais(montant) + " " + monnaie, 0.2f, BaseColor.LIGHT_GRAY, null, Element.ALIGN_RIGHT, Font_TexteSimple_Gras));
    ...
    public static void main(String[] a) {
        //Exemple
        DocumentPDFFacture docpdf = new DocumentPDFFacture(null, ACTION_OUVRIR, false, null);
    }
