 ...
 * @author user
 */
public enum ProcesseurNombreFacture {
    ...
    CALCULATE() {
                ...
                public String getLettres(double montant, String monnaie) throws Exception {
                    double montantArr = UtilFacture.round(montant, 2);
                    int montantArrInt = (int) montantArr;

                    double montantArrIntApVirg = montant - montantArrInt;
                    montantArrIntApVirg = UtilFacture.round(montantArrIntApVirg, 2);
    ...
    protected ProcesseurNombreFacture before;
    // valeur à ajout à la fin d'un nombre entier
    private String addMin;
    /* constructeurs*/

    ProcesseurNombreFacture() {

    }

    ProcesseurNombreFacture(long v, String s) {
        this(v, v, s);
    }

    ProcesseurNombreFacture(long min, long max) {
        this.min = min;
        this.max = max;
    }

    ProcesseurNombreFacture(long min, long max, String label, ProcesseurNombreFacture before) {
        this(min, max, label);
        this.before = before;
    }

    ProcesseurNombreFacture(long min, long max, String label, String addMin) {
        this(min, max, label);
        this.addMin = addMin;
    }

    ProcesseurNombreFacture(long min, long max, String label) {
        this(min, max);
        this.label = label;
    }
    ...
    private String getStringValue(long value) {
        ...
        ProcesseurNombreFacture[] values = ProcesseurNombreFacture.values();
        if (value < 20) {
            return values[(int) value].label;
        }
        for (int i = 0; i < values.length; i++) {
            ProcesseurNombreFacture nombre = values[i];
            ...
            if (value < 100 && nombre.min <= value && nombre.max >= value) {
                ...
                //100 et 1000
                if ((this.equals(MILLE) || this.equals(CENT)) && ProcesseurNombreFacture.UN.equals(nombre)) {
                    add.append(label);
                } else {
